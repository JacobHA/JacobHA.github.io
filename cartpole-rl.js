const canvas = document.getElementById('cartpole-rl');
...
function drawGame() {
    ...
    // Just write the current state: cart position and velocity, pole angle and angular velocity
    ctx.fillText(`Cart position: ${cartX.toFixed(2)}`, 10, 80);
    ctx.fillText(`Cart velocity: ${cartSpeed.toFixed(2)}`, 10, 120);
    ctx.fillText(`Pole angle: ${poleAngle.toFixed(2)}`, 10, 160);
    ctx.fillText(`Pole angular velocity: ${poleAngularVelocity.toFixed(2)}`, 10, 200);
